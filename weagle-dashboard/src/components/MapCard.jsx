...
import { Expand, Minus, PackageSearch, Plus, Search } from 'lucide-react'
...
function MapCard() {
  ...
  return (
    <div className="map-card">
      <div className="map-header"><div className="track-input"><Search size={20} strokeWidth={2} /><input type="text" placeholder="Enter track number" value={trackNumber} onChange={(e) => setTrackNumber(e.target.value)} /></div><div className="map-controls"><button className="map-control-btn"><Plus size={20} strokeWidth={2} /></button><button className="map-control-btn"><Minus size={20} strokeWidth={2} /></button><button className="map-control-btn"><Expand size={20} strokeWidth={2} /></button></div></div>
      <div className="map-container">
        ...
        <div className="map-overlay"><div className="status-panel"><div className="panel-tabs"><div className="tab active">Status</div><div className="tab">Routes</div><div className="tab">Alerts</div></div><div className="status-overview">{rows.map((row) => <div key={row.label} className="status-bar"><span className="status-label">{row.label}</span><div className="status-fill"><div className="status-fill-inner" style={{ width: row.width, '--status-color': tokenColor(row.token) }}></div></div><span className="status-count">{row.count}</span></div>)}</div></div>
          <div className="ledger-panel"><div className="panel-tabs"><div className="tab active">Recent Shipments Ledger</div></div><div className="ledger-list">{['Electronics Package', 'Food Delivery', 'Document Express'].map((title, index) => <div key={title} className="ledger-item"><PackageSearch size={20} strokeWidth={2} /><div className="ledger-info"><div className="ledger-title">{title}</div><div className="ledger-time">Nov 15, 2026</div></div><div className="status-dot" style={{ '--status-color': tokenColor(rows[index].token) }}></div></div>)}</div><button className="view-full-btn">View Full</button></div></div>
      </div>
    </div>
  )
}
